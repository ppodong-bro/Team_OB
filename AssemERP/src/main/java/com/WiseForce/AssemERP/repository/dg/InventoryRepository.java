...
public interface InventoryRepository extends JpaRepository<Month_Inventory, Month_Inventory_ID> {
	// 이번 월 기말재고의 종류 수 조회
	@Query("SELECT count(mi0) FROM Month_Inventory mi0 "
			+ "WHERE mi0.id.yearmonth = (SELECT MAX(mi1.id.yearmonth) FROM Month_Inventory mi1) "
			+ "AND mi0.id.startend_status = 1 " // 기말 재고
			+ "AND mi0.id.item_status = :item_status") // 부품
	int getLastestMonthInventoryCnt(@Param("item_status") int item_status);
