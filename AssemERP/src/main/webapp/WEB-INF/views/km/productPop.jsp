...
<body>
	...
	<!-- 검색 폼 (거래처 팝업과 동일 UI) -->
	<form method="get" action="<c:url value='/sales/productPopup'/>"
		class="mb-2">
		<div class="input-group input-group-sm">
			<input type="text" class="form-control" name="product_Name"
				value="${fn:escapeXml(param.product_Name)}" placeholder="제품명 검색" />
	...
	<table class="table table-sm table-hover align-middle table-bordered">
		<thead class="table-light">
			<tr>
				<th style="width: 110px" class="text-center">제품코드</th>
		...
		<tbody>
			<c:forEach var="p" items="${productList}">
				...
				<tr>
					<td class="text-center">${p.product_no}</td>
					<td class="text-center">${fn:escapeXml(p.product_name)}</td>
					<td class="text-center">
						<button type="button" class="btn btn-primary btn-sm selectBtn"
							...
							data-product-version="${fn:escapeXml(ver)}">선택</button>
					</td>
				</tr>
			</c:forEach>
			...
			<c:if test="${empty productList}">
				<tr>
					<td colspan="4" class="text-center text-muted">결과가 없습니다.</td>
				</tr>
			</c:if>
		</tbody>
	</table>
	...
	<script>
		...
		document.addEventListener('click', function(e) {
			...
			if (window.opener && !window.opener.closed
					&& typeof window.opener.setProductInfo === 'function') {
				window.opener.setProductInfo(d.productNo, d.productName,
						d.productVersion);
			}
			window.close();
		});
